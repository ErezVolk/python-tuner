#!/usr/bin/env python3
"""A minimal command-line guitar/ukulele tuner in Python."""
######################################################################
...
def note_to_fftbin(n):
    """Convert MIDI number to FFT bin(?)."""
...
def main():
    ...
    window = 0.5 * (
        1 - np.cos(np.linspace(0, 2 * np.pi, SAMPLES_PER_FFT, False))
    )
    ...
    while stream.is_active():
        ...
        if num_frames >= FRAMES_PER_FFT:
            print(
                f"freq: {freq:7.2f} Hz     "
                f"note: {note_name(n0):>3s} {n - n0:+.2f}"
            )
